...
load("F:/我的论文/第五篇/主代码/beta anomaly/monthly data in five years/RData/da_funding_5y.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_m <- merge(da_all_m,da_funding_5y,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(funding_1),]
  #da_sub <- da_sub[order(funding_2),]
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(funding_1)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(funding_1,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(funding_2)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(funding_2,size)]
  }
}
