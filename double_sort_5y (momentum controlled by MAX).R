#################################
#### Double Sort with Momentum Controlled by MAX
...
####
da_m <- merge(da_all_m[,.(ym,SecCode,max_ret,ret_e,size)],da_beta_5y,by=c("ym","SecCode"))
da_m <- merge(da_m,da_mom_m,by=c("ym","SecCode")) # for mom

da_m[,mom_max_ret:=summary(lm(mom ~ max_ret))$residuals,by=ym]
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(mom_max_ret),]
