...
da_m_lm[illiq!=0,`:=`(size=log(size),BM=log(BM),illiq=log(illiq),rvol=log(rvol))]
...
####  
lm_1 <- lmList(be ~ rvol | ym , data=da_m_lm)
da_vari_m <- da_m_lm[,.(ym,ret_e,be,candidate=rvol)]
FUN_DECOMPOSITION(lm_1)

lm_2 <- lmList(be ~ rsk | ym , data=da_m_lm)
da_vari_m <- da_m_lm[,.(ym,ret_e,be,candidate=rsk)]
